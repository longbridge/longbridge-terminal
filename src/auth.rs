...
pub async fn auth_code_exchange_login(code: &str) -> Result<()> {
    ...
    for candidate in &candidates {
        // The code carries its dynamically-registered client_id as a JWT claim;
        // candidates that are not such a JWT can't be exchanged this way.
        let Some(client_id) = client_id_from_code(candidate) else {
            continue;
        };
        saw_client_id = true;
