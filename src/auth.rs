...
/// Redirect URI registered for the "AI Agent" client. Deliberately a fixed
/// constant (NOT region-aware): it is never navigated to — it only has to
/// byte-match the value the connect page sends to `/v1/authorize/confirm`
/// and the single URI registered on the client, regardless of which regional
/// site generated the code.
fn agent_redirect_uri() -> String {
    "https://open.longbridge.com/connect/done".to_string()
}
...
/// <https://open.longbridge.com/connect> (10-minute, single-use) — for an
