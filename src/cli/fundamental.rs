...
fn print_insider_holding(data: &Value) {
    let empty = vec![];
    let records = data["insider_holdings"].as_array().unwrap_or(&empty);
    if records.is_empty() {
        println!("No data.");
        return;
    }
    let headers = &["Period", "Price", "Price Chg", "Buy", "Sell"];
    let rows = records
        .iter()
        .map(|e| {
            vec![
                val_str(&e["period"]),
                val_str(&e["stock_price"]),
                val_str(&e["percent_stock_price_changed"]),
                val_str(&e["accum_buy"]),
                val_str(&e["accum_sell"]),
            ]
        })
        .collect();
    super::output::print_table(headers, rows, &OutputFormat::Pretty);
...
/// Fetch company insider holdings (executives and directors).
pub async fn cmd_insider_holding(
    symbol: String,
    ...
    let data = http_get("/v1/quote/company_insider_holdings", &params, verbose).await?;
    match format {
        OutputFormat::Json => print_json(&data),
        OutputFormat::Pretty => print_insider_holding(&data),
    }
    Ok(())
}
